...
function refreshStats() {
  ...
  const avgConv   = entries.length ? (entries.reduce((s,e)=>s+(e.conviction||0),0)/entries.length).toFixed(1) : '—';
  document.getElementById('stat-positions').textContent  = positions.filter(p=>p.status==='open').length;
...
function renderRecentEntries() {
  ...
  if (!entries.length) {
    container.innerHTML = `<div class="empty-state"><div class="empty-state-icon">✦</div><p class="empty-state-title">No entries yet</p><p class="empty-state-text">Start documenting your investment thesis.</p><br/><a href="journal.html" class="btn btn-primary">Write First Entry</a></div>`;
    return;
  }
  ...
  container.innerHTML = items.map(w => {
    ...
    return `<div class="watch-snap-card">
      <div style="display:flex;align-items:center;gap:6px;margin-bottom:4px;">
        <span class="watch-snap-ticker">${escHtml(w.ticker)}</span>${exchangeBadge(w.exchange)}
      </div>
      <div class="watch-snap-name">${escHtml(w.name)}</div>
      <div class="watch-snap-prices">
        <div><span style="font-family:var(--font-mono);font-size:.6rem;color:var(--text-muted);text-transform:uppercase;letter-spacing:.1em">Current</span><br>
          <span style="font-family:var(--font-mono);font-size:.875rem;color:var(--text-primary)">${w.price||'—'}</span></div>
        <div style="text-align:right"><span style="font-family:var(--font-mono);font-size:.6rem;color:var(--text-muted);text-transform:uppercase;letter-spacing:.1em">Target</span><br>
          <span style="font-family:var(--font-mono);font-size:.875rem;color:var(--gold)">${w.target||'—'}</span></div>
      </div>
