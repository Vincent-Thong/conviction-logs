...
function renderRecentEntries() {
  ...
  if (!entries.length) {
    container.innerHTML = '<div class="empty-state"><div class="empty-state-icon">✦</div><p class="empty-state-title">No entries yet</p><p class="empty-state-text">Start documenting your investment thesis.</p><br/><a href="' + (loggedIn?'journal.html':'login.html') + '" class="btn btn-primary">' + (loggedIn?'Write First Entry':'Sign In to Write') + '</a></div>';
    return;
  }
  container.innerHTML = entries.map(function(e) {
    return '<a class="entry-row" href="journal.html">' +
...
function renderWatchlistSnapshot() {
  ...
  container.innerHTML = items.map(function(w) {
    ...
    return '<div class="watch-snap-card">' +
      ...
      '<span class="watch-snap-ticker">' + escHtml(w.ticker) + '</span>' + exchangeBadge(w.exchange) + '</div>' +
